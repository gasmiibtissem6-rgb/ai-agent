...
class MediaItem {
  ...
  String caption;
  String date;
  MediaItem({required this.type, required this.data, this.thumbnail, this.caption = '', required this.date});
}
...
class _DocumentsScreenState extends State<DocumentsScreen> {
  ...
  void _addMedia({bool videoOnly = false}) {
    final input = html.FileUploadInputElement();
    input.accept = videoOnly ? 'video/*' : 'image/*,video/*';
    input.multiple = true;
    input.click();
    input.onChange.listen((e) async {
      ...
      for (final file in files) {
        ...
        String? thumb;
        if (isVideo) {
          thumb = await _extractVideoThumbnail(base64);
        }
        setState(() {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('${files.length} fichier(s) ajouté(s)'), backgroundColor: Colors.green));
    });
  }

  Future<String?> _extractVideoThumbnail(String videoBase64) async {
    try {
  ...
  void _downloadPdf() async {
    try {
      final dio = Dio();
      final mediaPayload = _mediaItems.map((m) => {
        'type': m.type,
  ...
  Widget build(BuildContext context) {
    ...
    return IdealAppScaffold(
      ...
      body: IdealGradientBackground(
        child: _scannedText.isEmpty ? _buildImportScreen(cs) : _buildEditScreen(cs),
      ),
    );
  }
      ...
      child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          if (_isScanning)
            ...
            _buildCard(cs, Icons.camera_alt, 'Prendre une photo', 'Appareil photo ou fichier image', Colors.blue, () => _pickFile(imageOnly: true)),
            const SizedBox(height: 16),
            _buildCard(cs, Icons.upload_file, 'Importer un fichier', 'PDF ou image depuis votre appareil', Colors.green, () => _pickFile()),
            const SizedBox(height: 16),
            _buildCard(cs, Icons.edit_document, 'Nouveau contrat vide', 'Rédiger depuis zéro', Colors.purple, () {
              setState(() { _scannedText = ' '; _editController.text = ''; });
            }),
          ],
        ]),
      ),
      ...
      Container(
        ...
        child: Row(children: [
          IconButton(icon: const Icon(Icons.arrow_back),
            onPressed: () => setState(() { _scannedText = ''; _mediaItems.clear(); _showSignature = false; _signatureImageBase64 = null; })),
            ...
            if (_showSignature) ...[
              ...
              Container(
                ...
                child: Row(children: [
                  ...
                  Text('Document signé électroniquement', style: TextStyle(color: Colors.green.shade700, fontWeight: FontWeight.bold)),
                  const Spacer(),
                  if (_signatureImageBase64 != null)
                    ClipRRect(borderRadius: BorderRadius.circular(4),
                      child: Image.network(_signatureImageBase64!, height: 40, fit: BoxFit.contain)),
                ]),
              ),
            ],
          ]),
        ...
        Expanded(
          child: Stack(fit: StackFit.expand, children: [
            ClipRRect(borderRadius: const BorderRadius.vertical(top: Radius.circular(12)),
              child: Image.network(previewData, fit: BoxFit.cover)),
            if (item.type == 'video')
            ...
            Positioned(top: 4, right: 4,
              child: GestureDetector(
                ...
                child: Container(decoration: const BoxDecoration(color: Colors.red, shape: BoxShape.circle),
                  padding: const EdgeInsets.all(2), child: const Icon(Icons.close, size: 14, color: Colors.white)))),
          ]),
        ),
        Padding(
          ...
          child: Row(children: [
            Expanded(child: TextField(
              ...
              decoration: const InputDecoration(isDense: true, border: InputBorder.none, hintText: 'Légende...'),
            )),
...
class _SignatureDialogState extends State<_SignatureDialog> {
  ...
  Future<String> _captureDrawing() async {
    try {
      final canvas = html.CanvasElement(width: 400, height: 180);
      ...
      for (int i = 0; i < _points.length - 1; i++) {
        if (_points[i] != null && _points[i + 1] != null) {
          ctx.moveTo(_points[i]!.dx, _points[i]!.dy);
          ctx.lineTo(_points[i + 1]!.dx, _points[i + 1]!.dy);
        }
      }
      ctx.stroke();
      ...
      child: Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Container(
            ...
            child: Row(children: [_tabBtn('✍️ Dessiner', 0), _tabBtn('📷 Caméra', 1)]),
          ),
          const SizedBox(height: 12),
          if (_tab == 0)
            Container(
                ...
                child: GestureDetector(
                  ...
                  child: CustomPaint(
                    ...
                    child: _points.isEmpty
                        ? Center(child: Text('Dessinez votre signature', style: TextStyle(color: Colors.grey.shade400)))
                        : null,
                  ),
                ),
              ),
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  SizedBox(height: 8),
                  Text('Importer une photo de votre signature', textAlign: TextAlign.center),
                  SizedBox(height: 4),
                  Text('(signez sur papier blanc, prenez une photo)', style: TextStyle(fontSize: 11, color: Colors.grey)),
                ])),
              ),
            )
          ...
          const SizedBox(height: 12),
          Row(children: [
            TextButton(onPressed: () => setState(() { _points = []; _hasSignature = false; _cameraImageBase64 = null; }),
