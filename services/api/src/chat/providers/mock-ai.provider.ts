...
export class MockAiProvider implements AiProvider {
  async chat(request: AiChatRequest): Promise<AiChatResponse> {
    ...
    if (
      ...
      lastAiMessage.includes('Question 5/5') ||
      ...
      lastAiMessage.includes('Option A') ||
      lastAiMessage.includes('Option B') ||
      msg.includes('option a') ||
      msg.includes('option b') ||
      msg.includes('option c')
    ) {
      const allContent =
        history.map((h) => h.content).join(' ') + ' ' + request.message;
      const prestataire =
        allContent
          .match(/prestataire[^.]*?:(.*?)(?:client|\.)/i)?.[1]
          ?.trim() ??
        'Ahmed Ben Salah, auto-entrepreneur, 15 rue de Carthage Tunis, MF 123456A';
      const client =
        allContent.match(/client[^.]*?:(.*?)(?:service|\.)/i)?.[1]?.trim() ??
        'Société Delta SARL, capital 50 000 DT, avenue Mohamed V Tunis, MF 789012B, représentée par Sami Trabelsi';
